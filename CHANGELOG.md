...
**问题：**
1. 眼睛区域在像素化后不明显 — LANCZOS 下采样将眼睛黑像素与周围肤色平均，生成灰色而非深色
2. 左右眼不对称 — 边缘检测和颜色量化各自独立，导致眼睛一深一浅

**方案：**

**对比度感知采样（`pixelate.py`）**
- 重写 `pixelate()`，逐网格格子采样原始像素而非整体 LANCZOS 缩放
- 新增 `_sample_cell()`：计算格子内亮度范围，若高对比度（暗部 < 100 且跨度 > 25），取亮度底部 10%~20% 像素的平均值而非全局平均
- 确保即使暗部像素占比很小（如 14×14 格子中仅 5% 为眼睛），格子仍使用深色

**对称性强制（`pixelate.py` + `main.py`）**
- 新增 `_enforce_symmetry()`：在边缘覆盖之后，遍历每一行，左右对称像素取更暗者
- 将对称性放在边缘覆盖之后执行，确保最终输出完全对称

**边缘参数调整**
- `threshold_percentile`：80 → 70（更敏感，覆盖更多结构线）
- 默认 `edge_strength`：0.35 → 0.4（结构线更深）

**涉及文件：**
- `backend/app/processors/pixelate.py` — 重写核心，新增 `_sample_cell()` / `_enforce_symmetry()`
- `backend/app/main.py` — 对称性置于边缘覆盖之后，参数默认值更新
- `frontend/src/App.tsx` — `edge_strength` 默认值 0.35 → 0.4

**效果：**
- 21×28 网格中眼睛呈现为清晰对称的深色块（亮度 34，接近纯黑）
- 全图不对称像素：0
- 眼睛区域完全左右对称
